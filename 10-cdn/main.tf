resource "aws_cloudfront_distribution" "roboshop_cf_distribution" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.roboshop_cf_distribution.domain_name
        zone_id = aws_cloudfront_distribution.roboshop_cf_distribution.hosted_zone_id
      }
    }
  ]
}
